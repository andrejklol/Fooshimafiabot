...
_log = logging.getLogger("bot.error_embed")
...
async def send_error_embed(
    ...
    # Dashboard-only: direct Discord channel posting disabled.
    # Kept function signature intact so existing callers continue to work;
    # event is logged locally for debugging.
    try:
        _log.log(
            logging.ERROR if level == "error" else logging.INFO,
            "[%s] %s | %s | user=%s actor=%s trace=%s extra=%s",
            level.upper(),
            title,
            description,
            username,
        ...
        print(f"Failed to log error embed: {e}")
